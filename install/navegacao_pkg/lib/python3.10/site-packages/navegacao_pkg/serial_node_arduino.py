...
class Ros2Serial(Node):
    ...
    def get_encoder_values(self, data):
        try:
            self.encoder_left_front  = float(data["encoders"][0] / (self.pulses_per_rotation * 2 * self.motor_reduction))
            self.encoder_right_front = - float(data["encoders"][1] / (self.pulses_per_rotation * 2 * self.motor_reduction))
            self.encoder_left_rear   = float(data["encoders"][2] / (self.pulses_per_rotation * 2 * self.motor_reduction))
            self.encoder_right_rear  = - float(data["encoders"][3] / (self.pulses_per_rotation * 2 * self.motor_reduction))
